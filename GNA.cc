...
GNA::GNA(ComponentId_t id, Params &params)
  ...
  if (!memory) {
    ...
    memory = loadAnonymousSubComponent<Interfaces::SimpleMem>(
        "memHierarchy.memInterface", "memory", 0, ComponentInfo::SHARE_PORTS,
        params, clockTC,
        new Interfaces::SimpleMem::Handler<GNA>(this, &GNA::handleEvent));
  }
  if (!memory)
    out.fatal(CALL_INFO, -1,
              "Unable to load memHierarchy.memInterface subcomponent\n");
}
...
void GNA::init(unsigned int phase) {
  ...
  for (int n = 0; n < numNeurons; ++n) {
    ...
    for (int nn = 0; nn < numCon; ++nn) {
      ...
      SimpleMem::Request *req = new SimpleMem::Request(
          SimpleMem::Request::Write, reqAddr, sizeof(T_Wme));
...
#else
  int bwpl_len = 2;
  Ctrl_And_Stat_Types::T_BwpFl *bwpl = (Ctrl_And_Stat_Types::T_BwpFl *)calloc(
      bwpl_len, sizeof(Ctrl_And_Stat_Types::T_BwpFl));
  ...
  for (int i = 0; i < bwpl_len; ++i) {
    BWPs.insert(
        std::pair<uint, Ctrl_And_Stat_Types::T_BwpFl>(bwpl[i].TmpSft, bwpl[i]));
  }
}
...
void GNA::handleEvent(Interfaces::SimpleMem::Request *req) {
  ...
  if (i == requests.end()) {
    out.fatal(CALL_INFO, -1,
              "Request ID (%" PRIx64 ") not found in outstanding requests!\n",
              req->id);
  } else {
    // handle event
